    ...
    try:
        if username and password:
            ...
            raise EnvironmentError(
                "Provide username and password as function arguments."
            )
...
def get_authenticated_user_projects(
    username, password, api_url="https://api.taiga.io/"
):
    """
    ...
    try:
        ...
        if hasattr(projects, "__len__") and len(projects) == page_size:
            clog(
                "warning",
                f"Project list may be truncated: found {len(projects)} projects (page size limit). There may be more projects for user {user.id}.",
            )
        if projects:
