...
def digest_schedule(url, username, password):
    ...
    try:
        ...
        for cal in calendars:
            try:
                ...
                for event in events:
                    try:
                        ...
                        if event_start.date() == tomorrow.date():
                            ...
                            events_day_after.append(
                                summary.value if summary else "No summary"
                            )
                        elif event_start.date() == two_days_after.date():
                            events_two_days_after.append(
                                summary.value if summary else "No summary"
                            )
                ...
                if "VCARD and VCALENDAR" in str(e):
                    clog(
                        "warning",
                        f"Skipping non-calendar resource: {getattr(cal, 'name', repr(cal))} (not a VCALENDAR)",
                    )
                else:
        ...
        lines = []
        lines.append(
            f"The users planned schedule for tomorrow, {tomorrow.strftime('%A, %B %d, %Y')}:\n"
        )
        if events_tomorrow:
        ...
        lines.append(
            f"The following is supplementary information that you shall only use when it has influence on the planning for tomorrow:"
        )
        lines.append(f"\nEvents in 2 days:")
        if events_day_after:
...
if __name__ == "__main__":
    ...
    if not (CALDAV_URL and CALDAV_USERNAME and CALDAV_PASSWORD):
        clog(
            "error",
            "Please set CALDAV_URL, CALDAV_USERNAME, and CALDAV_PASSWORD environment variables.",
        )
    else:
        try:
